...
export default function SquatTrackerPage() {
  const VIDEO_W = 480;
  const VIDEO_H = 360;
  ...
  const DOWN_RATIO = 0.6; // squats: down position at 60% of standing length
  const UP_RATIO = 1.0;   // up position at initial standing length
  const DELTA = 0.07;
  ...
  useEffect(() => {
    ...
    async function calibratePose() {
      if (
        ...
        if (
          kp.leftShoulder?.score > 0.7 && kp.leftAnkle?.score > 0.7 &&
          kp.rightShoulder?.score > 0.7 && kp.rightAnkle?.score > 0.7
        ) {
          leftDistances.current.push(dist(leftShoulder, leftAnkle));
          rightDistances.current.push(dist(rightShoulder, rightAnkle));
        }
        ...
        if (
          ...
          setStarted(true);
          setCalibrating(false);
          toast.success("Calibration complete! Start your squats.", { icon: "🏁", id: "initial" });
        } else {
    ...
    async function detectPose() {
      if (
        ...
        if (canvasRef.current) {
          ...
          const kp = pose.keypoints.reduce((acc, k) => { acc[k.part] = k; return acc; }, {});
          if (kp.leftShoulder && kp.leftAnkle && kp.leftShoulder.score > 0.7 && kp.leftAnkle.score > 0.7) {
            drawLine(kp.leftShoulder.position, kp.leftAnkle.position);
          }
          if (kp.rightShoulder && kp.rightAnkle && kp.rightShoulder.score > 0.7 && kp.rightAnkle.score > 0.7) {
            drawLine(kp.rightShoulder.position, kp.rightAnkle.position);
          }
        }
        ...
        if (
          ...
          const leftDist = dist(leftShoulder, leftAnkle);
          const rightDist = dist(rightShoulder, rightAnkle);
          ...
          if (isActive) {
            ...
            if (dormant && !resumeToastRef.current) {
              ...
              dormantToastRef.current = false;
              toast("Resumed squat detection.", { icon: "🔄", id: "resume" });
            }
          }
          ...
          if (ratio > UP_RATIO - DELTA && down && lastActiveDownRef.current) {
            ...
            if (now - lastCountTimeRef.current > COUNT_COOLDOWN) {
              setCount((c) => c + 1);
              setDown(false);
              toast(`Squat counted! Total: ${count + 1}`, { icon: "🏋️", id: `squat-${count + 1}` });
  ...
  return (
    <div style={{
      ...
      <h2 style={{ fontSize: "2rem", marginBottom: 8, color: "#000", gridArea:"2/1" }}>
        Squats: {count}
      </h2>
      <button
            style={{
      ...
      {!started && !calibrating && net &&
        <div style={{ margin: "22px 0 10px 0" }}>
          ...
          <div style={{ marginTop: 10, fontSize: "1.1rem" }}>
            After pressing Start, stand straight and keep still for a few seconds!
          </div>
        </div>
      }
      {calibrating && (
                      ...
                      <>
                        <a
                          ...
                          style={{
                            ...
                            ...(pathname === "/app/exercise" ? { background: "#112244" } : {})
                          }}
                        >
                          Pushup
                </a>
      ...
      <div style={{
        ...
        <ul>
          ...
          <li>Press <b>Start</b>, then <b>stand tall and keep still</b> for calibration.</li>
          <li>After you see "Calibration complete", begin your squats!</li>
          <li>Each full squat (down and up) counts as one rep.</li>
          <li>Move smoothly, avoid pausing mid-rep or moving too fast.</li>
          <li>Green dots and lines show detected body parts and skeleton. Toasts confirm each detected part and every squat counted.</li>
      ...
      {dormant && (
        <div style={{
          ...
          Dormant mode: No motion detected. Move to resume squat detection.
        </div>
      )}
      </li>
      </ol>
